...
public class BankDatabaseManager {
    ...
    private static final String CREATE_TRANSACTION_TABLE = "CREATE TABLE "
            ...
            + " INTEGER PRIMARY KEY,"
            + BankDatabaseSchema.TransactionRecord.COLUMN_NAME_TYPE + TEXT_TYPE
            + COMMA + BankDatabaseSchema.TransactionRecord.COLUMN_NAME_AMOUNT
            + REAL_TYPE + COMMA
            + BankDatabaseSchema.TransactionRecord.COLUMN_NAME_DATE + TEXT_TYPE
            + " )";
    ...
    public long addTransactionRecord(String type, double amount, String date) {
        ...
        if (date == null || date == "") {
            throw new IllegalArgumentException(
                    "The type of transaction date is NOT null and NOT empty!");
        } else {
